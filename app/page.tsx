...
import type {
  GraphData as VisionGraphData,
  Node as VisionNode,
  Edge as VisionEdge,
} from "@/components/VisionContextGraph"; // Renaming for clarity
...
export default function Home() {
  ...
  const [visionGraph, setVisionGraph] = useState<VisionGraphData>({
    nodes: [],
    edges: [],
  });
  const [llmReasoning, setLlmReasoning] = useState<LLMReasoningNode | null>(
    null,
  );
  ...
  const [selectedPacketEvent, setSelectedPacketEvent] =
    useState<PacketEvent | null>(null);
    ...
    const ws = initWebSocket({
      ...
      onMessage: (event) => {
        try {
          ...
          if (
            serverMessage.type === "system_message" &&
            serverMessage.event === "shutdown"
          ) {
            setSystemLogs((prevLogs) => [
              ...prevLogs,
              {
          ...
          if (serverMessage.type === "live_update") {
            ...
            if (data.vlm_analysis && data.vlm_analysis.scene_features) {
              ...
              console.log(
                "Scene Features for Vision Graph:",
                data.vlm_analysis.scene_features,
              );
            ...
            if (data.llm_reasoning) {
              ...
              const reasoningRoot: LLMReasoningNode = {
                ...
                children: [
                  {
                    id: `desc-${data.frame_id}-${crypto.randomUUID()}`,
                    name: `Scene: ${llmData.scene_description || "N/A"}`,
                    value: llmData.scene_description || "N/A",
                  },
                  ...(llmData.contextual_insights?.map(
            ...
            if (
              data.final_ios_response_summary &&
              data.final_ios_response_summary.error
            ) {
              setSystemLogs((prevLogs) => [
                ...prevLogs,
                {
            ...
            if (data.server_status) {
              ...
              setLlmProcessingStats(
                data.server_status.llm_processing_stats || null,
              );
              setVisionProcessingStats(
                data.server_status.vision_processing_stats || null,
              );
            }
            ...
            console.warn(
              "Received WebSocket message with unknown type or missing payload:",
              serverMessage,
            );
            setSystemLogs((prevLogs) => [
              ...prevLogs,
          ...
          console.error(
            "Error parsing WebSocket message or handling data:",
            error,
          );
          setSystemLogs((prevLogs) => [
            ...prevLogs,
  ...
  return (
    <main className="flex flex-col h-screen bg-background text-foreground font-[family-name:var(--font-geist-sans)]">
      <header className="h-16 border-b border-border bg-card flex items-center px-6 shrink-0">
        <img src="/orion.svg" alt="Orion Logo" className="h-8 w-8 mr-3" />
        <h1 className="text-xl font-semibold text-foreground">
          Orion Vision Dashboard
        </h1>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-full h-full max-w-4xl max-h-[70vh]">
              {processingMode === 'full' ? (
                ...
                <div className="bg-muted border rounded-lg h-full w-full flex items-center justify-center overflow-hidden">
                  <p className="text-muted-foreground">Image data is not sent in split mode.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <aside className="w-1/3 min-w-[300px] max-w-[500px] bg-card border-l p-4 flex flex-col gap-4 overflow-y-auto">
          ...
          <div className="flex-1 min-h-[300px]">
            <LLMReasoningTree
              reasoningData={llmReasoning}
              historicalContext={llmReasoning?.historicalContext}
            />
          </div>
          <div className="flex-1 min-h-[300px]">
