...
class DeviceManager: ObservableObject {
    ...
    func consumePairingCode(code: String) async throws -> DevicePair {
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            if let httpResponse = response as? HTTPURLResponse, let data = try? JSONDecoder().decode(ApiError.self, from: data) {
                throw NSError(domain: "APIError", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: data.error])
            }
            throw URLError(.badServerResponse)
        }
    ...
    func fetchPairedDevices() async {
        do {
            ...
            print("Error decoding paired devices:", decodingError)
