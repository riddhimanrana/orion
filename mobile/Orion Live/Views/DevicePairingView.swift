...
struct DevicePairingView: View {
    ...
    private func startPollingTimer() {
        pollingTimer = Timer.scheduledTimer(withTimeInterval: 5, repeats: true) { _ in
            Task {
                let initialPairCount = await MainActor.run { deviceManager.pairedDevices.count }
                await deviceManager.fetchPairedDevices()
                let newPairCount = await MainActor.run { deviceManager.pairedDevices.count }
                if newPairCount > initialPairCount {
