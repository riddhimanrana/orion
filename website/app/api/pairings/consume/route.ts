...
export async function POST(request: Request) {
  return trackApiRoute(request, { action: "pairings.consume" }, async (usage) => {
    ...
    usage.setUserId(consumerUser.id);
    ...
    const { data: creatorDeviceData, error: creatorDeviceError } =
      await supabase
        .from("devices")
        .select("user_id, type")
    ...
    const mobileDevice =
      creatorDeviceData.type === "ios"
        ? creatorDeviceData
        : consumerDeviceData;
    const serverDevice =
      creatorDeviceData.type === "mac"
        ? creatorDeviceData
        : consumerDeviceData;
    ...
    if (existingPair) {
      usage.addMetadata({ pairId: existingPair.id, reusedPair: true });
    ...
    usage.addMetadata({ pairId: pairData.id, reusedPair: false });
