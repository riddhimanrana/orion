...
export async function POST(request: Request) {
  ...
  const { data: { user: consumerUser }, error: getUserError } = await supabase.auth.getUser();
  ...
  if (!code || !consumer_device_id) {
    return new NextResponse(JSON.stringify({ error: "code and device_id are required" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
    });
  }
  ...
  const { data: consumerDeviceData, error: consumerDeviceError } = await supabase
    .from('devices')
    .select('id, type')
  ...
  if (consumerDeviceError || !consumerDeviceData) {
    return new NextResponse(JSON.stringify({ error: "Consumer device not found or access denied" }), {
        status: 403,
        headers: { "Content-Type": "application/json" },
    });
  }
  ...
  if (codesError) {
    ...
    return new NextResponse(JSON.stringify({ error: "Error fetching pairing codes" }), { status: 500 });
  }
  ...
  if (creatorDeviceError || !creatorDeviceData) {
    return new NextResponse(JSON.stringify({ error: "Creator device not found" }), { status: 500 });
  }

  // Check if both devices belong to the same user
  if (creatorDeviceData.user_id !== consumerUser.id) {
    return new NextResponse(JSON.stringify({ error: "Devices must belong to the same user" }), {
        status: 403,
        headers: { "Content-Type": "application/json" },
    });
  }
  ...
  const mobileDevice = creatorDeviceData.type === 'ios' ? creatorDeviceData : consumerDeviceData;
  const serverDevice = creatorDeviceData.type === 'mac' ? creatorDeviceData : consumerDeviceData;
  const mobile_device_id = creatorDeviceData.type === 'ios' ? creator_device_id : consumer_device_id;
  const server_device_id = creatorDeviceData.type === 'mac' ? creator_device_id : consumer_device_id;

  if (mobileDevice.type !== 'ios' || serverDevice.type !== 'mac') {
      return new NextResponse(JSON.stringify({ error: "Pairing must be between an iOS and a macOS device" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
    });
  }
  ...
  const { data: existingPair, error: existingPairError } = await supabase
    .from('device_pairs')
    .select('*, mobile_device:devices!mobile_device_id(*), server_device:devices!server_device_id(*)')
  ...
  if (pairError) {
    ...
    if (pairError.code === '23505') { // unique_violation
        return new NextResponse(JSON.stringify({ error: "A pair for these devices already exists." }), { status: 409 }); // 409 Conflict
    }
    return new NextResponse(JSON.stringify({ error: "Could not create device pair" }), { status: 500 });
  }
  ...
  if (updateCodeError) {
    ...
    console.error("CRITICAL: Failed to mark pairing code as used:", updateCodeError);
  }
