...
export async function GET(request: NextRequest) {
  ...
  if (!finalRedirectTo || !finalRedirectTo.startsWith("orionauth://")) {
  ...
  callbackUrl.searchParams.set('expires_in', data.session.expires_in.toString());
  callbackUrl.searchParams.set('token_type', 'bearer');
  
  console.log("Mobile bridge - Final redirect URL:", callbackUrl.toString());
  console.log("Mobile bridge - Session access token (first 20 chars):", data.session.access_token.substring(0, 20));
